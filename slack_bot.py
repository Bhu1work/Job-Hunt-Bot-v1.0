...
def _trigger_apply_pipeline(say) -> None:
    ...
    say(
        ...
        f"Phase 2 (Claude tailor) → Phase 3 (email find) → cold emails sent.\n"
        f"I'll ping you when done. Job IDs: `{ids_arg}`"
    )
    subprocess.Popen(
        ["python", "main.py", "--phase", "2,3,send", "--job-ids", ids_arg],
        cwd="/app",
    )
...
def notify_new_jobs(count: int, top_jobs: list[dict]) -> None:
    """Post a new-jobs notification to the configured Slack channel."""
    if not config.SLACK_BOT_TOKEN or not config.SLACK_CHANNEL_ID:
        return
    try:
        from slack_sdk import WebClient
        client = WebClient(token=config.SLACK_BOT_TOKEN)
        lines = [f":new: *{count} new jobs scraped!* Top picks:\n"]
        ...
        lines.append("\nUse `/jobs` in Slack to select which ones to apply to.")
